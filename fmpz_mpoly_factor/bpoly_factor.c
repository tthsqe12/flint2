...
next_prime:
    ...
    for (i = 0; i < I->r; i++)
    {
        fmpz_mod_poly_set_fmpz_poly(I->Bitilde1 + i, Bevalf->p + i);
        fmpz_mod_poly_make_monic(I->Bitilde1 + i, I->Bitilde1 + i);
        fmpz_mod_poly_set_fmpz_poly(I->Bitilde + i, Bevalf->p + i);
        fmpz_mod_poly_make_monic(I->Bitilde + i, I->Bitilde + i);
        fmpz_mod_bpoly_set_polyx(I->newBitilde + i, I->Bitilde + i);
    }
    ...
    for (i = 0; i < I->r; i++)
    {
        fmpz_mod_poly_set_fmpz_poly(Blead, B->coeffs + B->length - 1);
        fmpz_mod_bpoly_set_polyy(tryme, Blead);
        fmpz_mod_bpoly_mul(trymet, tryme, I->newBitilde + i, I->lifting_prec);
        fmpz_mod_bpoly_swap(trymet, tryme);
        fmpz_bpoly_set_fmpz_mod_bpoly(trymez, tryme);
        fmpz_bpoly_make_primitive(g, trymez);
        if (!fmpz_bpoly_divides(Q, B, trymez))
