...
main(void)
{
    ...
    if (0) {
        ...
        nmod_poly_randtest_irreducible(modulus, state, 10 + 1);
...
flint_printf("\nmodulus: "); nmod_poly_print_pretty(modulus, "#"); flint_printf("\n");
        ...
        for (i = 0; i < n; i++)
        {
            ...
            n_poly_fq_randtest(a + i, state, deg, ctx);
        ...
        for (j = 0; j < 4; j++)
        {
            timeit_start(timer);
            for (i = 0; i + 1 < n; i++)
                n_poly_fq_gcd_(b, a + i, a + i + 1, ctx, St);
            timeit_stop(timer);
            t1 = timer->wall;
            flint_printf("\n  new: %wd\n", t1);
            ...
            timeit_start(timer);
            for (i = 0; i + 1 < n; i++)
                fq_nmod_poly_gcd_euclidean(B, A + i, A + i + 1, ctx);
            timeit_stop(timer);
            t2 = timer->wall;
            flint_printf("  old: %wd\n", t2);
