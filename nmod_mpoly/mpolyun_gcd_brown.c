...
choose_prime: /* primes are v - alpha, v + alpha */
    ...
    /* evaluation point should kill neither A nor B */
    nmod_mpolyn_intp_reduce_2sm_poly(Aevalp, Aevalm, A, alphapow, ctx);
    nmod_mpolyn_intp_reduce_2sm_poly(Bevalp, Bevalm, B, alphapow, ctx);
    FLINT_ASSERT(Aevalp->length > 0);
    FLINT_ASSERT(Aevalm->length > 0);
    FLINT_ASSERT(Bevalp->length > 0);
    FLINT_ASSERT(Bevalm->length > 0);
    ...
    {
        slong Gdeg;
        nmod_mpolyn_intp_reduce_2sm_poly(Gevalp, Gevalm, G, alphapow, ctx);
    ...
    if (nmod_poly_degree(modulus) > 0)
    {
        ...
        if (!gstab)
        {
            gstab = !nmod_mpolyn_intp_crt_2sm_poly(&ldegG, G, T, Gevalp, Gevalm, modulus, alphapow, ctx);
        }
        nmod_mpolyn_intp_crt_2sm_poly(&ldegAbar, Abar, T, Abarevalp, Abarevalm, modulus, alphapow, ctx);
        nmod_mpolyn_intp_crt_2sm_poly(&ldegBbar, Bbar, T, Bbarevalp, Bbarevalm, modulus, alphapow, ctx);
    }
    else
    {
        nmod_mpolyn_intp_lift_2sm_poly(&ldegG, G, Gevalp, Gevalm, alpha, ctx);
        nmod_mpolyn_intp_lift_2sm_poly(&ldegAbar, Abar, Abarevalp, Abarevalm, alpha, ctx);
        nmod_mpolyn_intp_lift_2sm_poly(&ldegBbar, Bbar, Bbarevalp, Bbarevalm, alpha, ctx);
        gstab = astab = bstab = 0;
    }
    temp = nmod_mul(alpha, alpha, ctx->ffinfo->mod);
    nmod_poly_scalar_mul_nmod(modulus2, modulus, temp);
...
/* optimized version of nmod_mpolyn_gcd_brown_smprime_ref */
...
/*
flint_printf("nmod_mpolyn_gcd_brown_smprime called var = %wd\n", var);
...
choose_prime:
    ...
    /* evaluation point should kill neither A nor B */
    nmod_mpolyn_intp_reduce_2sm_mpolyn(Aevalp, Aevalm, A, var, alphapow, ctx);
    nmod_mpolyn_intp_reduce_2sm_mpolyn(Bevalp, Bevalm, B, var, alphapow, ctx);
    FLINT_ASSERT(Aevalp->length > 0);
    FLINT_ASSERT(Aevalm->length > 0);
    FLINT_ASSERT(Bevalp->length > 0);
    FLINT_ASSERT(Bevalm->length > 0);
    ...
    if (nmod_poly_degree(modulus) > 0)
    {
        ...
        changed = new_nmod_mpolyn_intp_crt_2sm_mpolyn(&ldegG, G, T1, Gevalp, Gevalm, var, modulus, alphapow, ctx);
        if (!changed && nmod_poly_degree(modulus) < upper_limit)
        {
...
successful_fix_lc:
        ...
        changed = new_nmod_mpolyn_intp_crt_2sm_mpolyn(&ldegAbar, Abar, T1, Abarevalp, Abarevalm, var, modulus, alphapow, ctx);
        if (!changed && nmod_poly_degree(modulus) < upper_limit)
        {
        ...
        changed = new_nmod_mpolyn_intp_crt_2sm_mpolyn(&ldegBbar, Bbar, T1, Bbarevalp, Bbarevalm, var, modulus, alphapow, ctx);
        if (!changed && nmod_poly_degree(modulus) < upper_limit)
        {
    ...
    {
        new_nmod_mpolyn_intp_lift_2sm_mpolyn(&ldegG, G, Gevalp, Gevalm, var, alpha, ctx);
        new_nmod_mpolyn_intp_lift_2sm_mpolyn(&ldegAbar, Abar, Abarevalp, Abarevalm, var, alpha, ctx);
        new_nmod_mpolyn_intp_lift_2sm_mpolyn(&ldegBbar, Bbar, Bbarevalp, Bbarevalm, var, alpha, ctx);
    }
...
/*
flint_printf("nmod_mpolyn_gcd_brown_smprime returning %d\n", success);
printf("   G: "); nmod_mpolyn_print_pretty(G, NULL, ctx); printf("\n");
printf("Abar: "); nmod_mpolyn_print_pretty(Abar, NULL, ctx); printf("\n");
printf("Bbar: "); nmod_mpolyn_print_pretty(Bbar, NULL, ctx); printf("\n");
*/
    return success;
}
...
have_prime:
    ...
    /* reduction should kill neither A nor B */
    nmod_mpolyn_intp_reduce_lg_poly(Aeval, ectx->fqctx, A, ctx);
    nmod_mpolyn_intp_reduce_lg_poly(Beval, ectx->fqctx, B, ctx);
    FLINT_ASSERT(Aeval->length > 0);
    FLINT_ASSERT(Beval->length > 0);
    ...
    {
        FLINT_ASSERT(G->length > 0);
        if (fq_nmod_poly_degree(Geval, ectx->fqctx) > ((G->exps + N*0)[off]>>shift))
        {
            goto choose_prime;
        }
        else if (fq_nmod_poly_degree(Geval, ectx->fqctx) < ((G->exps + N*0)[off]>>shift))
        {
            nmod_poly_one(modulus);
        }
    }
    ...
    if (nmod_poly_degree(modulus) > 0)
    {
        nmod_mpolyn_intp_crt_lg_poly(&ldegG, G, T, modulus, ctx, Geval, ectx->fqctx);
        nmod_mpolyn_intp_crt_lg_poly(&ldegAbar, Abar, T, modulus, ctx, Abareval, ectx->fqctx);
        nmod_mpolyn_intp_crt_lg_poly(&ldegBbar, Bbar, T, modulus, ctx, Bbareval, ectx->fqctx);
    }
    else
    {
        nmod_mpolyn_intp_lift_lg_poly(&ldegG, G, ctx, Geval, ectx->fqctx);
        nmod_mpolyn_intp_lift_lg_poly(&ldegAbar, Abar, ctx, Abareval, ectx->fqctx);
        nmod_mpolyn_intp_lift_lg_poly(&ldegBbar, Bbar, ctx, Bbareval, ectx->fqctx);
    }
    ...
    const nmod_mpoly_ctx_t ctx)
{
    ...
    if (var == 1)
    {
        success = nmod_mpolyn_gcd_brown_lgprime_bivar(G, Abar, Bbar, A, B, ctx);
        return success;
    }
...
have_prime:
    ...
    /* make sure reduction does not kill either A or B */
    nmod_mpolyn_intp_reduce_lg_mpolyn(Aeval, ectx, A, var, ctx);
    nmod_mpolyn_intp_reduce_lg_mpolyn(Beval, ectx, B, var, ctx);
    ...
    if (nmod_poly_degree(modulus) > 0)
    {
        new_nmod_mpolyn_intp_crt_lg_mpolyn(&ldegG, G, T, modulus, var, ctx, Geval, ectx);
        new_nmod_mpolyn_intp_crt_lg_mpolyn(&ldegAbar, Abar, T, modulus, var, ctx, Abareval, ectx);
        new_nmod_mpolyn_intp_crt_lg_mpolyn(&ldegBbar, Bbar, T, modulus, var, ctx, Bbareval, ectx);
    }
    else
    {
        new_nmod_mpolyn_intp_lift_lg_mpolyn(&ldegG, G, var, ctx, Geval, ectx);
        new_nmod_mpolyn_intp_lift_lg_mpolyn(&ldegAbar, Abar, var, ctx, Abareval, ectx);
        new_nmod_mpolyn_intp_lift_lg_mpolyn(&ldegBbar, Bbar, var, ctx, Bbareval, ectx);
    }
    nmod_poly_mul(modulus, modulus, ectx->fqctx->modulus);
