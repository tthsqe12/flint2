...
_new_splitbase_struct;

typedef _new_splitbase_struct _new_splitbase_t[1];

typedef struct
{
    slong idx;
    _new_splitbase_struct * base;
    nmod_mpolyn_t G, Abar, Bbar;
    nmod_poly_t modulus;
    mp_limb_t alpha;
    slong required_images;
}
_new_splitworker_arg_struct;

static void _new_splitworker_bivar(void * varg)
{
    _new_splitworker_arg_struct * arg = (_new_splitworker_arg_struct *) varg;
    _new_splitbase_struct * base = arg->base;
    const nmod_mpoly_ctx_struct * ctx = base->ctx;
    nmod_poly_t modulus2, alphapow, r;
    nmod_poly_t Aevalp, Bevalp, Gevalp, Abarevalp, Bbarevalp;
    nmod_poly_t Aevalm, Bevalm, Gevalm, Abarevalm, Bbarevalm;
    ...
    while (nmod_poly_degree(arg->modulus) < arg->required_images)
    {
        ...
        /* evaluation point should kill neither A nor B */
        nmod_mpolyn_intp_reduce_2sm_poly(Aevalp, Aevalm,
                                                       base->A, alphapow, ctx);
        nmod_mpolyn_intp_reduce_2sm_poly(Bevalp, Bevalm,
                                                       base->B, alphapow, ctx);
        FLINT_ASSERT(Aevalp->length > 0);
        FLINT_ASSERT(Aevalm->length > 0);
        FLINT_ASSERT(Bevalp->length > 0);
        ...
        if (use_stab && gstab)
        {
            ...
            nmod_mpolyn_intp_reduce_2sm_poly(Gevalp, Gevalm,
                                                        arg->G, alphapow, ctx);
        ...
        if (nmod_poly_degree(arg->modulus) > 0)
        {
            ...
            if (!gstab)
            {
                gstab = !nmod_mpolyn_intp_crt_2sm_poly(&ldeg, arg->G, T,
                                 Gevalp, Gevalm, arg->modulus, alphapow, ctx);
            }
            nmod_mpolyn_intp_crt_2sm_poly(&ldeg, arg->Abar, T,
                            Abarevalp, Abarevalm, arg->modulus, alphapow, ctx);
            nmod_mpolyn_intp_crt_2sm_poly(&ldeg, arg->Bbar, T,
                            Bbarevalp, Bbarevalm, arg->modulus, alphapow, ctx);
        }
        else
        {
            nmod_mpolyn_intp_lift_2sm_poly(&ldeg, arg->G,
                                                   Gevalp, Gevalm, alpha, ctx);
            nmod_mpolyn_intp_lift_2sm_poly(&ldeg, arg->Abar,
                                             Abarevalp, Abarevalm, alpha, ctx);
            nmod_mpolyn_intp_lift_2sm_poly(&ldeg, arg->Bbar,
                                             Bbarevalp, Bbarevalm, alpha, ctx);
            gstab = astab = bstab = 0;
        }
        temp = nmod_mul(alpha, alpha, ctx->ffinfo->mod);
...
static void _new_splitworker(void * varg)
{
    _new_splitworker_arg_struct * arg = (_new_splitworker_arg_struct *) varg;
    _new_splitbase_struct * base = arg->base;
    const nmod_mpoly_ctx_struct * ctx = base->ctx;
    ...
    while (nmod_poly_degree(arg->modulus) < arg->required_images)
    {
        ...
        /* evaluation should kill neither A nor B */
        nmod_mpolyn_intp_reduce_2sm_mpolyn(Aevalp, Aevalm,
                                                  base->A, var, alphapow, ctx);
        nmod_mpolyn_intp_reduce_2sm_mpolyn(Bevalp, Bevalm,
                                                  base->B, var, alphapow, ctx);
        FLINT_ASSERT(Aevalp->length > 0);
        FLINT_ASSERT(Aevalm->length > 0);
        FLINT_ASSERT(Bevalp->length > 0);
        ...
        if (nmod_poly_degree(arg->modulus) > 0)
        {
            ...
            temp = n_invmod(temp, ctx->ffinfo->mod.n);
            nmod_poly_scalar_mul_nmod(arg->modulus, arg->modulus, temp);
            new_nmod_mpolyn_intp_crt_2sm_mpolyn(&ldeg, arg->G, T,
                             Gevalp, Gevalm, var, arg->modulus, alphapow, ctx);
            new_nmod_mpolyn_intp_crt_2sm_mpolyn(&ldeg, arg->Abar, T,
                       Abarevalp, Abarevalm, var, arg->modulus, alphapow, ctx);
            new_nmod_mpolyn_intp_crt_2sm_mpolyn(&ldeg, arg->Bbar, T,
                       Bbarevalp, Bbarevalm, var, arg->modulus, alphapow, ctx);
        }
        else
        {
            new_nmod_mpolyn_intp_lift_2sm_mpolyn(&ldeg, arg->G,
                                              Gevalp, Gevalm, var, alpha, ctx);
            new_nmod_mpolyn_intp_lift_2sm_mpolyn(&ldeg, arg->Abar,
                                        Abarevalp, Abarevalm, var, alpha, ctx);
            new_nmod_mpolyn_intp_lift_2sm_mpolyn(&ldeg, arg->Bbar,
                                        Bbarevalp, Bbarevalm, var, alpha, ctx);
        }
        temp = nmod_mul(alpha, alpha, ctx->ffinfo->mod);
        nmod_poly_scalar_mul_nmod(modulus2, arg->modulus, temp);
...
static slong _new_nmod_mpolyn_crt(
    const nmod_poly_multi_crt_t P,
    _new_joinworker_arg_struct * S,
    nmod_mpolyn_struct * const * B,
    slong count,
    nmod_poly_struct * output,
    nmod_poly_struct ** input,
...
typedef _new_joinbase_struct _new_joinbase_t[1];

typedef struct
{
    _new_joinbase_struct * base;
    slong thread_idx;
}
_njoinworker_arg_struct;

/* Join the images of G, Abar, and Bbar in each of the split threads */
static void _new_joinworker(void * varg)
{
    _njoinworker_arg_struct * arg = (_njoinworker_arg_struct *) varg;
    _new_joinbase_struct * base = arg->base;
    nmod_poly_struct ** input;
    nmod_poly_struct * output;
    ...
    while (1)
    {
        ...
        {
            base->chunks[i].lastdeg = _new_nmod_mpolyn_crt(base->CRT,
...
static void _new_finaljoinworker(void * varg)
{
    _njoinworker_arg_struct * arg = (_njoinworker_arg_struct *) varg;
    _new_joinbase_struct * base = arg->base;
    const nmod_mpoly_ctx_struct * ctx = base->ctx;
    ...
    slong num_handles)
{
    ...
    nmod_poly_struct ** mptrs;
    nmod_mpolyn_struct ** gptrs, ** abarptrs, ** bbarptrs;
    _new_splitworker_arg_struct * splitargs;
    _new_splitbase_t splitbase;
    _njoinworker_arg_struct * joinargs;
    _new_joinbase_t joinbase;
    nmod_poly_t t1;
    nmod_mpolyn_t T1, T2;
#if WANT_ASSERT
...
printf("nmod_mpolyn_gcd_brown_smprime_threaded called!!!!!!!\n");
    ...
    mptrs = (nmod_poly_struct **) flint_malloc(
                                       num_threads*sizeof(nmod_poly_struct *));
    splitargs = (_new_splitworker_arg_struct *) flint_malloc(
                                  num_threads*sizeof(_new_splitworker_arg_struct));
    for (i = 0; i < num_threads; i++)
    {
...
compute_split:
    ...
    {
        thread_pool_wake(global_thread_pool, handles[i],
                  var == 1 ? _new_splitworker_bivar : _new_splitworker, &splitargs[i]);
    }
    (var == 1 ? _new_splitworker_bivar : _new_splitworker)(&splitargs[num_threads - 1]);
    for (i = 0; i + 1 < num_threads; i++)
    {
        thread_pool_wait(global_thread_pool, handles[i]);
    }
    ...
    */
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        {
            _new_joinworker_arg_struct * d = joinbase->chunks + i*num_threads + j;
    ...
    {
        thread_pool_wake(global_thread_pool,
                                    handles[i], _new_joinworker, joinargs + i);
    }
    _new_joinworker(joinargs + num_threads - 1);
    for (i = 0; i + 1 < num_threads; i++)
    {
        thread_pool_wait(global_thread_pool, handles[i]);
    }
...
*/
    ...
    {
        thread_pool_wake(global_thread_pool,
                                   handles[i], _new_finaljoinworker, joinargs + i);
    }
    _new_finaljoinworker(joinargs + num_threads - 1);
    for (i = 0; i + 1 < num_threads; i++)
    {
        thread_pool_wait(global_thread_pool, handles[i]);
    }
    ...
    slong thread_limit)
{
    ...
    if (num_handles > 0)
    {
        ...
        thread_pool_wake(global_thread_pool, handles[m], _new_worker_convertn, arg);
