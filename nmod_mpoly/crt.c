...
static void nmod_poly_crt_fit_length(nmod_poly_crt_t P, slong k)
{
    ...
    if (P->alloc == 0)
    {
        ...
        P->prog = (_prog_instr *) flint_malloc(k*sizeof(_prog_instr));
        P->alloc = k;
    }
    else if (k > P->alloc)
    {
        FLINT_ASSERT(P->prog != NULL);
        P->prog = (_prog_instr *) flint_realloc(P->prog, k*sizeof(_prog_instr));
        P->alloc = k;
    }
}
...
*/
static slong _push_prog(nmod_poly_crt_t P, nmod_poly_struct ** moduli, slong * perm,
...
*/
void nmod_poly_crt_run(const nmod_poly_crt_t P, nmod_poly_struct ** outputs,
                                                nmod_poly_struct ** inputs)
{
    slong i;
    slong a, b, c;
