    ...
    mpoly_rbnode_t node)
{
    ...
    fq_zech_mpoly_swap(A->coeffs + A->length, (fq_zech_mpoly_struct *)(node->data), NULL);
    A->length++;

    fq_zech_mpoly_clear(node->data, NULL);
...
#define LUT_limit (48)
    ...
    if (bits <= FLINT_BITS)
    {
        ...
        for (i = 0; i < Blen; i++)
        {
            ...
            else
            {
                ...
                if (new)
                {
                    d = flint_malloc(sizeof(fq_zech_mpoly_struct));
        ...
        /* clear out tree to A */
        fq_zech_mpoly_univar_fit_length(A, tree->size + LUT_limit, ctx);
        A->length = 0;
        if (tree->size > 0)
    ...
    else
    {
        ...
        /* clear out tree to A */
        fq_zech_mpoly_univar_fit_length(A, tree->size, ctx);
        A->length = 0;
