...
slong _fq_zech_mpoly_add(fq_zech_struct * coeff1,       ulong * exp1,
                   ...
                   slong N, const ulong * cmpmask, const fq_zech_ctx_t fqctx)
{
    ...
    while (i < len2 && j < len3)
    {
        ...
        {
            mpoly_monomial_set(exp1 + k*N, exp2 + i*N, N);
            fq_zech_add(coeff1 + k, coeff2 + i, coeff3 + j, fqctx);
        ...
        else
        {
            ...
            fq_zech_set(coeff1 + k, coeff3 + j, fqctx);
            j++;
        }
        k++;
    }
...
void fq_zech_mpoly_add(fq_zech_mpoly_t poly1, const fq_zech_mpoly_t poly2,
                          const fq_zech_mpoly_t poly3, const fq_zech_mpoly_ctx_t ctx)
{
    ...
    if (poly2->length == 0)
    {
        fq_zech_mpoly_set(poly1, poly3, ctx);
        return;
    }
    ...
    if (poly1 == poly2 || poly1 == poly3)
    {
        ...
        len1 = _fq_zech_mpoly_add(temp->coeffs, temp->exps, 
    ...
    else
    {
        ...
        len1 = _fq_zech_mpoly_add(poly1->coeffs, poly1->exps, 
                       poly2->coeffs, exp2, poly2->length,
                       poly3->coeffs, exp3, poly3->length,
                                    N, cmpmask, ctx->fqctx);
    }
