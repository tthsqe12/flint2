...
slong _fq_nmod_mpoly_divrem_monagan_pearce(slong * lenr,
...
return 0;
    ...
    while (heap_len > 1)
    {
        ...
        if (!lt_divides)
        {
            _fq_nmod_mpoly_fit_length(&r_coeff, &r_exp, allocr, r_len + 1, N, fqctx);
            fq_nmod_neg(r_coeff + r_len, q_coeff + q_len, fqctx);
...
FLINT_ASSERT(0);
    ...
    if (r == poly2 || r == poly3)
    {
        fq_nmod_mpoly_init2(temp2, poly3->length, ctx);
    ...
    {
        fq_nmod_mpoly_fit_length(r, poly3->length, ctx);
    ...
    while ((lenq = _fq_nmod_mpoly_divrem_monagan_pearce(&lenr, &tq->coeffs, &tq->exps,
