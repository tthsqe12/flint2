...
void
fmpz_mod_poly_randtest_irreducible(fmpz_mod_poly_t f, flint_rand_t state,
                                           slong len, const fmpz_mod_ctx_t ctx)
{
    ...
    } while (fmpz_mod_poly_is_zero(f) || !fmpz_mod_poly_is_irreducible(f, ctx));
}
...
void
fmpz_mod_poly_randtest_trinomial(fmpz_mod_poly_t poly,
                       flint_rand_t state, slong len, const fmpz_mod_ctx_t ctx)
{
    ulong k;
    fmpz_mod_poly_fit_length(poly, len);
...
void
fmpz_mod_poly_randtest_pentomial(fmpz_mod_poly_t poly,
                       flint_rand_t state, slong len, const fmpz_mod_ctx_t ctx)
{
    fmpz_mod_poly_fit_length(poly, len);
    _fmpz_vec_zero(poly->coeffs, len);
