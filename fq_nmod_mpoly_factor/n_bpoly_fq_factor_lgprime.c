...
void n_bpoly_fq_set_n_fq_poly_var0_lg_to_sm(
    n_bpoly_t A,
    const n_poly_t E,
    const bad_fq_nmod_embed_t emb)
{
    ...
    const fq_nmod_ctx_t ctx)
{
    ...
    for (s = 1; s <= len/2; s++)
    {
        ...
        while (1)
        {
            ...
            n_bpoly_fq_set_n_fq_poly_var1(t1, f->coeffs + f->length - 1, ctx);
            for (i = 0; i < len; i++)
            {
                if (subset[i] >= 0)
                {
    ...
    const bad_fq_nmod_embed_t emb)
{
    ...
    for (j = 0; j < 2*r - 2; j++)
    {
        n_bpoly_fq_set_n_fq_poly_var0_lg_to_sm(v + j, V + j, emb);
        n_bpoly_fq_set_n_fq_poly_var0_lg_to_sm(w + j, W + j, emb);
    }
