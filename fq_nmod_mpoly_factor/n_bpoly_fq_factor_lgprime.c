...
void n_bpoly_fq_set_n_poly_fq_var0_lg_to_sm(
    n_bpoly_t A,
    const n_poly_t E,
    const bad_fq_nmod_embed_t emb)
{
...
void n_bpoly_fq_make_monic_mod_poly(
    n_bpoly_t A,
    const n_bpoly_t B,
    const n_poly_t m,
    const fq_nmod_ctx_t ctx)
{
    ...
    n_poly_fq_xgcd(g, lcinv, t, B->coeffs + B->length - 1, m, ctx);
    FLINT_ASSERT(n_poly_fq_is_one(g, ctx));

    n_bpoly_fit_length(A, B->length);
    for (i = 0; i < B->length; i++)
        n_poly_fq_mulmod(A->coeffs + i, B->coeffs + i, lcinv, m, ctx);
    ...
    const fq_nmod_ctx_t ctx)
{
    ...
    for (j = 0; j < C->length; j++)
    {
        ...
        n_poly_fq_rem(A->coeffs + i + j, A->coeffs + i + j, m, ctx);
    }
...
void n_bpoly_fq_divrem_mod_poly(
    n_bpoly_t Q,
    n_bpoly_t R,
    const n_bpoly_t A,
    const n_bpoly_t B,
    ...
    const fq_nmod_ctx_t ctx)
{
    ...
    n_poly_fq_xgcd(q, Binv, t, B->coeffs + B->length - 1, m, ctx);
    FLINT_ASSERT(n_poly_fq_is_one(q, ctx));

    while (R->length >= B->length)
    {
        n_poly_fq_mulmod(q, R->coeffs + R->length - 1, Binv, m, ctx);

        for (i = 0; i < B->length; i++)
        {
            n_poly_fq_mulmod(t, B->coeffs + i, q, m, ctx);
    ...
    const fq_nmod_ctx_t ctx)
{
    ...
    for (i = 0; i < r; i++)
    {
        n_bpoly_fq_divrem_mod_poly(Q, R, f, g[i], lift_alpha_pow, ctx);
        FLINT_ASSERT(R->length == 0);
        n_bpoly_fq_derivative(R, g[i], ctx);
        n_bpoly_fq_mul_mod_poly(ld + i, Q, R, lift_alpha_pow, ctx);
    }
    ...
    const fq_nmod_ctx_t ctx)
{
    ...
    for (i = 0; i < d; i++)
    {
        ...
        for (j = 0; j < r; j++)
        {
            ...
            n_bpoly_fq_mul_mod_poly(t1, loc_fac + i, loc_fac_org[j],
                                                         final_alpha_pow, ctx);
    ...
    for (s = 1; s <= len/2; s++)
    {
        ...
        while (1)
        {
            ...
            for (i = 0; i < len; i++)
            {
                if (subset[i] >= 0)
                {
                    n_bpoly_fq_mul_mod_poly(t2, t1, loc_fac + subset[i],
                                                         final_alpha_pow, ctx);
                    n_bpoly_swap(t1, t2);
                }
            }
    ...
    {
        n_tpoly_fit_length(F, F->length + 1);
        n_bpoly_fq_set(F->coeffs + F->length, f, ctx);
        F->length++;
    }
    else
    {
    ...
    const bad_fq_nmod_embed_t emb)
{
    ...
    for (i = 0; i < r; i++)
    {
        n_poly_fq_set_fq_nmod_poly(V + i, local_facs + i, emb->lgctx);
        link[i] = -i - 1;
    }
    ...
    for (i = r, j = 0; j < 2*r - 4; i++, j += 2)
    {
        ...
        n_poly_fq_mul(V + i, V + j, V + j + 1, emb->lgctx);
        link[i] = j;
    }

    for (j = 0; j < 2*r - 2; j += 2)
    {
        n_poly_fq_xgcd(d, W + j, W + j + 1, V + j, V + j + 1, emb->lgctx);
        FLINT_ASSERT(n_poly_fq_is_one(d, emb->lgctx));
    }

    for (j = 0; j < 2*r - 2; j++)
    {
        n_bpoly_fq_set_n_poly_fq_var0_lg_to_sm(v + j, V + j, emb);
        n_bpoly_fq_set_n_poly_fq_var0_lg_to_sm(w + j, W + j, emb);
    }
    ...
    const fq_nmod_ctx_t ctx)
{
    ...
    for (i = 0; i < c->length; i++)
    {
        n_poly_fq_divrem(tq, tr, c->coeffs + i, p0, ctx);
        FLINT_ASSERT(n_poly_fq_is_canonical(tr, ctx));
    ...
    for (i = 0; i < c->length; i++)
    {
        ...
        FLINT_ASSERT(n_poly_fq_is_canonical(tr, ctx));
        FLINT_ASSERT(n_poly_is_zero(tr));
        n_poly_fq_divrem(tr, c->coeffs + i, tq, p1, ctx);
    }

    n_bpoly_fq_mul_mod_poly(t1, c, b, p1, ctx);
    n_bpoly_fq_divrem_mod_poly(q, r, t1, g, p1, ctx);
    for (i = 0; i < r->length; i++)
        n_poly_fq_mul(r->coeffs + i, r->coeffs + i, p0, ctx);
    for (i = 0; i < g->length; i++)
        n_poly_fq_divrem(tq, g->coeffs + i, g->coeffs + i, p0, ctx);
    n_bpoly_fq_add(t1, r, g, ctx);

    n_bpoly_fq_mul_mod_poly(t2, c, a, p1, ctx);
    n_bpoly_fq_divrem_mod_poly(q, r, t2, h, p1, ctx);
    for (i = 0; i < r->length; i++)
        n_poly_fq_mul(r->coeffs + i, r->coeffs + i, p0, ctx);
    for (i = 0; i < h->length; i++)
        n_poly_fq_divrem(tq, h->coeffs + i, h->coeffs + i, p0, ctx);
...
#if WANT_ASSERT
    {
        n_poly_t p01;
        n_poly_init(p01);
        n_poly_fq_mul(p01, p0, p1, ctx);
        n_bpoly_fq_mul(t1, G, H, ctx);
        n_bpoly_fq_sub(c, f, t1, ctx);
        for (i = 0; i < c->length; i++)        
        {
            n_poly_fq_divrem(tq, tr, c->coeffs + i, p01, ctx);
            FLINT_ASSERT(n_poly_is_zero(tr));
        }
        n_poly_clear(p01);
    }
    ...
    const fq_nmod_ctx_t ctx)
{
    ...
    for (i = 0; i < b->length; i++)
        n_poly_fq_divrem(tq, b->coeffs + i, b->coeffs + i, p0, ctx);
    for (i = 0; i < a->length; i++)
        n_poly_fq_divrem(tq, a->coeffs + i, a->coeffs + i, p0, ctx);
...
*/
    ...
    *ctx_mock->fqctx = *ctx;
    ...
    n_poly_fq_set_fq_nmod_poly(alpha, cur_emb->h, ctx);
...
got_alpha:
    ...
    n_bpoly_fq_eval_sm_to_lg(Beval, B, cur_emb);
    ...
    n_poly_fq_pow(final_alpha_pow, alpha, final_pow, ctx);
    ...
    n_poly_fq_pow(curr_alpha_pow, alpha, curr_lift_pow, ctx);

    n_bpoly_fq_make_monic_mod_poly(monicB, B, curr_alpha_pow, ctx);
    ...
    for (i--; i > 0; i--)
    {
        n_poly_fq_pow(prev_alpha_pow, alpha, e[i+1], ctx);
        n_poly_fq_pow(p1, alpha, e[i]-e[i+1], ctx);
        _hensel_lift_tree(0, link, v, w, monicB, 2*r-4, prev_alpha_pow, p1, ctx);
    }

    prev_lift_pow = e[1];
    n_poly_fq_pow(prev_alpha_pow, alpha, prev_lift_pow, ctx);
    n_poly_fq_pow(p1, alpha, curr_lift_pow - prev_lift_pow, ctx);
...
more:
    ...
    n_poly_fq_pow(p1, alpha, next_lift_pow - curr_lift_pow, ctx);

    n_poly_fq_mul(next_alpha_pow, next_alpha_pow, p1, ctx);
    n_bpoly_fq_make_monic_mod_poly(monicB, B, next_alpha_pow, ctx);
