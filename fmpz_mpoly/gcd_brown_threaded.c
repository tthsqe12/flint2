...
static void _reduce_Bp_worker(void * varg)
{
    ...
    fmpz_mpoly_intp_reduce_p_mpolyn(arg->Bp, arg->pctx, arg->base->B,
                                                               arg->base->ctx);
}
...
static void _join_Abar_worker(void * varg)
{
    ...
    if (!fmpz_is_one(arg->modulus))
        new_fmpz_mpoly_intp_crt_p_mpolyn(arg->Abar, arg->T1, arg->base->ctx,
                                          arg->modulus, arg->Abarp, arg->pctx);
    else
        fmpz_mpoly_intp_lift_p_mpolyn(arg->Abar, arg->base->ctx,
                                                        arg->Abarp, arg->pctx);
}
...
static void _join_Bbar_worker(void * varg)
{
    _new_splitworker_arg_struct * arg = (_new_splitworker_arg_struct *) varg;
    if (!fmpz_is_one(arg->modulus))
        new_fmpz_mpoly_intp_crt_p_mpolyn(arg->Bbar, arg->T2, arg->base->ctx,
                                          arg->modulus, arg->Bbarp, arg->pctx);
    else
        fmpz_mpoly_intp_lift_p_mpolyn(arg->Bbar, arg->base->ctx,
                                                        arg->Bbarp, arg->pctx);
}
...
static void _new_splitworker(void * varg)
{
    ...
    while (arg->image_count < arg->required_images)
    {
        ...
        if (arg->num_handles > 0)
        {
            ...
            fmpz_mpoly_intp_reduce_p_mpolyn(arg->Ap, arg->pctx, base->A, ctx);
        ...
        else
        {
            ...
            fmpz_mpoly_intp_reduce_p_mpolyn(arg->Ap, arg->pctx, base->A, ctx);
            fmpz_mpoly_intp_reduce_p_mpolyn(arg->Bp, arg->pctx, base->B, ctx);
        ...
        if (arg->num_handles > 0)
        {
            ...
            if (!fmpz_is_one(arg->modulus))
                new_fmpz_mpoly_intp_crt_p_mpolyn(arg->G, arg->T, ctx, arg->modulus,
                                                           arg->Gp, arg->pctx);
            else
                fmpz_mpoly_intp_lift_p_mpolyn(arg->G, ctx, arg->Gp, arg->pctx);
        ...
        else
        {
            if (!fmpz_is_one(arg->modulus))
            {
                new_fmpz_mpoly_intp_crt_p_mpolyn(arg->G, arg->T, ctx,
                                             arg->modulus, arg->Gp, arg->pctx);
                new_fmpz_mpoly_intp_crt_p_mpolyn(arg->Abar, arg->T, ctx,
                                          arg->modulus, arg->Abarp, arg->pctx);
                new_fmpz_mpoly_intp_crt_p_mpolyn(arg->Bbar, arg->T, ctx,
                                          arg->modulus, arg->Bbarp, arg->pctx);
            }
            else
            {
                fmpz_mpoly_intp_lift_p_mpolyn(arg->G, ctx,
                                                           arg->Gp, arg->pctx);
                fmpz_mpoly_intp_lift_p_mpolyn(arg->Abar, ctx,
                                                        arg->Abarp, arg->pctx);
                fmpz_mpoly_intp_lift_p_mpolyn(arg->Bbar, ctx,
                                                        arg->Bbarp, arg->pctx);
            }
        }
...
static slong _new_fmpz_mpoly_crt(
    const fmpz_multi_crt_t P,
    ...
    */
    ...
    for (i = 1; i < num_master_threads; i++)
    {
        thread_pool_wake(global_thread_pool, splitargs[i].master_handle,
                                                 _new_splitworker, &splitargs[i]);
    }
    _new_splitworker(&splitargs[0]);
    for (i = 1; i < num_master_threads; i++)
    {
