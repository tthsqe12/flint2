...
main(void)
{
    ...
    for (i = 0; i < 10 * flint_test_multiplier(); i++)
    {
       ...
       for (j = 0; j < 4; j++)
       {
          ...
          fmpz_mpoly_quasidivrem_heap(s, k, r, h, g, ctx);
          fmpz_mpoly_test(k, ctx);
          fmpz_mpoly_test(r, ctx);

          result = fmpz_equal_ui(s, WORD(1)) && fmpz_mpoly_equal(f, k, ctx);
          ...
          if (!result)
          {
             ...
             printf(", len = %ld, exp_bits = %ld, exp_bound = %lx, "
                ...
                printf("h: ");fmpz_mpoly_print_pretty(h, NULL, ctx); printf("\n\n");
                printf("k: ");fmpz_mpoly_print_pretty(k, NULL, ctx); printf("\n\n");
    ...
    for (i = 0; i < 50 * flint_test_multiplier(); i++)
    {
       ...
       for (j = 0; j < 4; j++)
       {
            ...
            fmpz_mpoly_quasidivrem_heap(s, h, r, f, g, ctx);
            fmpz_mpoly_test(h, ctx);
            fmpz_mpoly_test(r, ctx);
            fmpz_mpoly_remainder_strongtest(r, g, ctx);

            fmpz_mpoly_mul_johnson(k, h, g, ctx);
            fmpz_mpoly_add(k, k, r, ctx);
            fmpz_mpoly_scalar_mul_fmpz(l, f, s, ctx);

            result = fmpz_mpoly_equal(l, k, ctx);
            ...
            if (!result)
            {
                ...
                printf("l: \n");fmpz_mpoly_print_pretty(l, NULL, ctx); printf("\n\n");
