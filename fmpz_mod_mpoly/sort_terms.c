    ...
    ulong totalmask)
{
    ...
    if ((totalmask & mask) == WORD(0))
    {
        ...
        {
            pos--;
            _fmpz_mod_mpoly_radix_sort1(A, left, right, pos, cmpmask, totalmask);
        }
        return;
    }
    ...
    while (++cur < right)
    {
        ...
        {
            fmpz_swap(A->coeffs + mid, A->coeffs + cur);
    ...
    {
        pos--;
        _fmpz_mod_mpoly_radix_sort1(A, left,  mid, pos, cmpmask, totalmask);
        _fmpz_mod_mpoly_radix_sort1(A, mid, right, pos, cmpmask, totalmask);
    }
}
    ...
    ulong * cmpmask)
{
    ...
    while (++check < right)
    {
        ...
        {
            fmpz_swap(A->coeffs + mid, A->coeffs + check);
    ...
    {
        pos--;
        _fmpz_mod_mpoly_radix_sort(A, left,  mid, pos, N, cmpmask);
        _fmpz_mod_mpoly_radix_sort(A, mid, right, pos, N, cmpmask);
    }
}
...
*/
void fmpz_mod_mpoly_sort_terms(
    fmpz_mod_mpoly_t A,
    const fmpz_mod_mpoly_ctx_t ctx)
{
    ...
    if (N == 1)
    {
        if (msb >= 0)
        {
            _fmpz_mod_mpoly_radix_sort1(A, 0, A->length, msb, ptempexp[0], himask);
        }
    }
    else
    {
        _fmpz_mod_mpoly_radix_sort(A, 0, A->length, (N - 1)*FLINT_BITS + msb, N, ptempexp);
    }
