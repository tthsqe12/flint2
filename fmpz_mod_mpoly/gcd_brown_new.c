    ...
    const fmpz_mod_mpoly_ctx_t ctx)
{
    ...
    if (fmpz_mod_poly_degree(modulus) < 1)
    {
        fmpz_mod_mpolyn_interp_lift_2sm_mpolyn(lastdeg, F, A, B, var,
                                                    alphapow->coeffs + 1, ctx);
    }

    FLINT_ASSERT(fmpz_mod_poly_degree(modulus)%2 == 0);
    ...
    fmpz_mod_poly_polyun_mpolyn_stack_t St)
{
    ...
    fmpz_mod_polyun_stack_fit_request(St->polyun_stack, 1);
    moduli = fmpz_mod_polyun_stack_take_top(St->polyun_stack);
...
choose_prime:
    ...
    if (i < 1)
    {
        fmpz_mod_polyun_fit_length(moduli, 1, ctx->ffinfo);
        fmpz_mod_poly_one(moduli->terms[i].coeffs, ctx->ffinfo);
        moduli->terms[i].exp = 0;
        moduli->length = ++i;
    }
    else if (moduli->terms[i - 1].coeff->length > limit)
    {
        /* make sure evaluation point does not kill both lc(A) and lc(B) */
        fmpz_mod_poly_gcd(t, gamma, moduli->terms[i - 1].coeff, ctx->ffinfo);
        if (fmpz_mod_poly_is_one(t, ctx->ffinfo)
        {
            fmpz_mod_polyun_fit_length(moduli, i + 1, ctx->ffinfo);
            fmpz_mod_poly_one(moduli->terms[i].coeffs, ctx->ffinfo);
            moduli->terms[i].exp = moduli->terms[i - 1].exp +
                                   moduli->terms[i - 1].coeff->length - 1;
            moduli->length = ++i;
        }
        else
        {
            fmpz_mod_poly_one(moduli->terms[i - 1].coeffs, ctx->ffinfo);
        }
    }
    fmpz_mod_mul(temp, alpha, outer_alpha, ctx->ffinfo);
    fmpz_mod_neg(temp, temp, ctx->ffinfo);
    fmpz_mod_poly_shift_left_scalar_addmul_fmpz_mod(moduli->terms[i - 1].coeff,
                                                         2, temp, ctx->ffinfo);

    if (bound >= moduli->terms[i - 1].exp +
                 moduli->terms[i - 1].coeff->length - 1)
    {
        goto choose_prime;
    }

    fmpz_mod_poly_multi_mod_precompute(Pmod, moduli->coeffs);

    fmpz_mod_mpolyn_interp_multi_mod(Amultimod, A, Pmod, ctx);
    fmpz_mod_mpolyn_interp_multi_mod(Bmultimod, B, Pmod, ctx);

    deg_bound_monomial

    for (i = 0; i < moduli->length; i++)
    {
        fmpz_mod_mpolyn_mock_multi_mod(Amock, Amultimod, moduli, i )
        fmpz_mod_mpolyn_mock_multi_mod(Bmock, Bmultimod, moduli, i)

        fmpz_mod_poly_one(modulus);

        for (alpha in roots(moduli[i]))
        {
            ...
            else if (eval_deg < bound)
            {
                bound = eval_deg
                remove all previous final_moduli (and {G,Abar,Bbar}mulimod)
                fmpz_mod_poly_one(modulus, ctx->ffinfo);
            }
        ...
        append modulus to final_moduli
        append G to Gmultimod
        append Abar to Gmultimod
        append Bbar to Gmultimod
    }
    ...
    fmpz_mod_poly_multi_crt_precompute(Pcrt, final_moduli);

    fmpz_mod_mpolyn_interp_multi_crt(&ldegG, G, Gmultimod, Pcrt, ctx);
    fmpz_mod_mpolyn_interp_multi_crt(&ldegAbar, Abar, Abarmultimod, Pcrt, ctx);
    fmpz_mod_mpolyn_interp_multi_crt(&ldegBbar, Bbar, Bbarmultimod, Pcrt, ctx);
